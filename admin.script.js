...
(async () => { 
    ...
    style.textContent = ` 
        body, div, p, h2, span, strong, button, li, label, .card-header, th.sortable, nav button, a, path, svg { 
            user-select: none; 
            -webkit-user-select: none; 
            -moz-user-select: none; 
            -ms-user-select: none; 
        } 
        table td, table th:not(.sortable), input, textarea, .channel-name, .user-info, #current-user-email { 
            user-select: text; 
...
async function showInventoryStatus() { 
            ...
            <div class="control-grid control-grid-inventory"> 
                <div class="card"> 
                    ...
                    <div class="card-body card-body-filters"> 
                        <input type="text" id="filter-inventory-location" class="filter-input" placeholder="로케이션 검색..." value="${currentFilters.location_code || ''}"> 
                        <input type="text" id="filter-inventory-pcode" class="filter-input" placeholder="상품코드 검색..." value="${currentFilters.product_code || ''}"> 
                        <input type="text" id="filter-inventory-barcode" class="filter-input" placeholder="바코드 검색..." value="${currentFilters.barcode || ''}"> 
                        <input type="text" id="filter-inventory-pname" class="filter-input" placeholder="상품명 검색..." value="${currentFilters.product_name || ''}"> 
                ...
                <div class="card danger-zone"> 
                    ...
                    <div class="card-body" style="flex-direction: column; align-items: stretch; gap: 1rem;"> 
                        ...
                        <div> 
                            ...
                            <div style="font-size: 0.8rem; color: #6c757d; margin-top: 0.25rem; margin-bottom: 0.5rem;"> 
                                CORN [재고 > 재고 조회 > 재고 현황 로케이션/상품별/LOT별 메뉴] 에서 엑셀다운 
                            </div> 
    ...
    if (currentFilters.location_code) { 
        const searchTerm = currentFilters.location_code.toLowerCase();
        filteredData = filteredData.filter(item => item.location_code.toLowerCase().includes(searchTerm)); 
    } 

    if (currentFilters.product_code) {
        const searchTerm = currentFilters.product_code.toLowerCase();
        filteredData = filteredData.filter(item => item.product_code && item.product_code.toLowerCase().includes(searchTerm));
    }

    if (currentFilters.barcode) {
        const searchTerm = currentFilters.barcode.toLowerCase();
        filteredData = filteredData.filter(item => item.barcode && item.barcode.toLowerCase().includes(searchTerm));
    }
    
    if (currentFilters.product_name) {
        const searchTerm = currentFilters.product_name.toLowerCase();
        filteredData = filteredData.filter(item => item.product_name && item.product_name.toLowerCase().includes(searchTerm));
    }
    // --- 여기까지 수정 ---
    ...
    contentArea.innerHTML = `<div id="products-section" class="content-section active"> 
        <div class="sticky-controls"> 
            ...
            <div class="control-grid"> 
                ...
                <div class="card"> 
                    <div class="card-header">CORN 양식 업로드</div> 
                    <div class="card-body" style="flex-direction: column; align-items: stretch; gap: 0.5rem;"> 
                        <div style="font-size: 0.8rem; color: #6c757d; text-align: left;"> 
                            CORN [기준정보 > 상품관리 > 상품정보조회] 에서 엑셀다운 
                        </div> 
                        <div class="upload-group"> 
        ...
        <div class="sticky-controls"> 
            ...
            <div class="control-grid"> 
                <div class="card"> 
                    <div class="card-header">필터 및 검색</div> 
                    <div class="card-body card-body-filters"> 
                        <input type="text" id="filter-loc-code" class="filter-input" placeholder="로케이션 검색..." value="${currentFilters.location_code || ''}"> 
                        <button class="search-button btn-primary">${icons.search} 검색</button> 
                        <button class="reset-button btn-secondary">${icons.reset} 초기화</button> 
                    </div> 
                </div> 
                ...
                <div class="card"> 
                    ...
                    <div class="card-body" style="flex-direction: column; align-items: stretch; gap: 0.5rem;"> 
                         <div style="font-size: 0.8rem; color: #6c757d; text-align: left;"> 
                             CORN [기준정보 > 물류센터관리 > 로케이션 관리 > 복수] 에서 엑셀다운 
                        </div> 
                        <div class="upload-group"> 
...
function downloadExcel(data, filename) {
    ...
    const namePart = filename.replace('.xlsx', '');
    const timeString = getFormattedDateString();
    const finalFilename = `${namePart}_${timeString}.xlsx`;
    ...
    keys.forEach(key => {
        ...
        data.forEach(row => {
            ...
            for (let i = 0; i < cellValue.length; i++) {
                cellLen += (cellValue.charCodeAt(i) > 128) ? 2.1 : 1.2;
            }
        ...
        for (let C = range.s.c; C <= range.e.c; ++C) {
            ...
            if (R === 0) {
                worksheet[address].s = {
                    ...
                    font: { bold: true, color: { rgb: "1A202C" } },
...
function downloadTemplateExcel(headers, filename) {
    ...
    const wscols = headers.map(header => ({ wch: header.length * 2 + 5 }));
    worksheet['!cols'] = wscols;
...
contentArea.addEventListener('click', async function(event) { 
    ...
    if (target.classList.contains('search-button')) { 
        ...
        if (sectionId === 'inventory-section') { 
            currentFilters.location_code = document.getElementById('filter-inventory-location').value.trim(); 
            currentFilters.product_code = document.getElementById('filter-inventory-pcode').value.trim();
            currentFilters.barcode = document.getElementById('filter-inventory-barcode').value.trim();
            currentFilters.product_name = document.getElementById('filter-inventory-pname').value.trim();
        // --- 여기까지 수정 ---
